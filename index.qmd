...
dim(yarn)
```
The data has 10,000 rows and 24 columns
...
```
The dataset yarn has a column 'n_washable', and the original data type was True/False. When taking a sum, R converts these values into intergers (1/0).'NA' is not able to be placed in either category, so we must remove the NA values, and then take a sum of the column. This answer is then stored in 'n_washable'.
...
4. Consider the column `yarn_company_name`.  
    ...
    3. (5 points) Is this data in tidy form? Explain why or why not.
     - The yarn data is in tidy form, there is a column for each variable, and a row for each observation(type of yarn), and a cell contains only one value. The 'common_yarn' data is similar, but it has only one ID variable (yarn_company_name) and one calculated variable, 'n', but this si still tidy.
...
# Execute the transformation you planned out in 4.4 to create a data set from `common_yarn` that has three columns: `yarn_company_name`, `yarn_weight_name`, and `n`, the number of entries from each company with that yarn weight. 
common_yarn_weight = (
    yarn[yarn['yarn_company_name'].isin(common_yarn['yarn_company_name'])]
    .groupby(['yarn_company_name', 'yarn_weight_name'])
    .size()
    .reset_index(name='n')
)
# Create a plot of the brand and yarn weight using a tile geometric object, mapping `n` to the fill color of the tile. If you cannot figure out the tile plot, you can use a scatterplot to make a similar (though less easily readable) plot, mapping point size to `n` instead. 
# -- 5) Plot a heatmap (tile plot) of counts
pivot = common_yarn_weight.pivot(
    index='yarn_company_name',
    columns='yarn_weight_name',
    values='n'
).fillna(0)
fig, ax = plt.subplots(figsize=(12, 8))
sns.heatmap(pivot, ax=ax, annot=True)
ax.set_xlabel('Yarn Weight Name')
ax.set_ylabel('Yarn Company Name')
ax.set_title('Counts of Yarn Weights by Top 10 Companies')
plt.tight_layout()
plt.show()
...
5. Consider the columns `texture` and `texture_clean`. 
    ...
    > Test to see if a descriptor in 'top_descriptors' is in texture_clean using 'str_detect'. If 'str_detect' does not look for a vector pattern, then loop through top_descriptors to see if ant of the keywords are present.
...
filter(yarn,texture != yarn$texture_clean)|>
...
descriptors <- yarn$texture_clean |>
  ...
  unlist()|>
  str_trim()
descriptors_count <- table(descriptors)|>
...
is_top_descriptors <- function(descr) {
...
yarn$common_texture <- is_top_descriptors(yarn$texture_clean)
...
# For how many rows of `yarn` do `texture` and `texture_clean` differ?
diff = (
    (yarn['texture'] != yarn['texture_clean']) &
    yarn['texture'].notna() &
    yarn['texture_clean'].notna()
)
num_diffs = diff.sum()
# Examine the rows where `texture` and `texture_clean` differ. What function(s) do you think were used to transform `texture` to `texture_clean`?
print(yarn.loc[diff, ['texture', 'texture_clean']].head(10))
# Create a variable, `top_descriptors`, that contains the top 15 common descriptor words/phrases.
descriptors = (
    yarn['texture_clean']
    .dropna()
...
descriptors_count = descriptors.value_counts()
top_descriptors = descriptors_count.index[:15].tolist()
print(top_descriptors)
# Write a function corresponding to the operations described in 5.5, `is_top_descriptor`. Your function should take a texture description and return `True` if one of the `top_descriptors` is present, and `False` if none of the `top_descriptors` are present. 
pattern = '|'.join(map(re.escape, top_descriptors)
def is_top_descriptors(descr):
    """
    Return True if `descr` contains any of the top descriptors, False otherwise.
    """
    if pd.isna(descr):
        return False
    return bool(re.search(pattern, descr))
  )
# Create a new column in `yarn`, `common_texture`, which is `True` if `texture_clean` contains one of the popular keywords and `False` otherwise. 
yarn['common_texture'] = yarn['texture_clean'].apply(is_top_descriptors)
print(yarn[['texture_clean', 'common_texture']].head(10))
```
...
6. Consider the variable `yarn_weight_wpi`.
    ...
    > 1. replace all "0-4" values with 4 and all "5-6" with values 6(use highest in rangem since range of values is disjointed)
...
convert_weight_int <- function(x) {
  x_numstr <- x |> 
    str_replace_all(pattern = c("0-4"= "4", "5-6"=6))
...
# Print out the unique values for `yarn_weight_wpi` in the `yarn` dataset. 
print(yarn['yarn_weight_wpi'].unique())
# Write a function, `convert_weight_int(x)` that takes a character string describing the weight and converts it to a representative integer.
def convert_weight_int(x):
    ...
    try:
        ...
        return np.nan
# Use your function to create a new column in yarn, `yarn_weight_wpi_num`. 
yarn['yarn_weight_wpi_num'] = yarn['yarn_weight_wpi'].apply(convert_weight_int)
# What is the mean yarn weight for Bernat yarns?
# If you had trouble with 6.4, you can use `prob6pt4.csv` to complete this problem. 
bernat_mean = (
    yarn
    .loc[yarn['yarn_company_name'] == 'Bernat', 'yarn_weight_wpi_num']
    .mean()
)
print(f"Mean yarn weight (WPI) for Bernat yarns: {bernat_mean:.2f}")
# Remove any rows with an NA for `yarn_weight_wpi_num`.     
# If you had trouble with 6.4, you can use `prob6pt4.csv` to complete this problem. 
yarn_no_na = yarn.dropna(subset=['yarn_weight_wpi_num'])
print(yarn_no_na.head(10))
```
